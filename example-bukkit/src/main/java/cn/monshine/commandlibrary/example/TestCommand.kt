...
object TestCommand {
    ...
    @JvmStatic
    @Command(names = ["ban"], permission = "op", description = "Ban someone", playerOnly = false)
    fun ban(sender: CommandSender, @Param(name = "s", type = ParamType.FLAG) silent: Boolean, @Param(name = "target") target: Player, @Param(name = "reason", wildcard = true) reason: String) {
